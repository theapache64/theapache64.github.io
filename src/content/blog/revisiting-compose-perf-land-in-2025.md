...
Back in 2023, I spent a considerable amount of time diving deep into Jetpack Compose performance. I built a decent understanding of how Compose works under the hood. Concepts like recomposition, compiler metrics, what makes a data model unstable, and why certain composable functions skip recomposition while others don't. These were things I actively tracked and optimized for.

Fast forward to today, and it's been nearly 2 years since that deep dive. The ecosystem has evolved, and I found myself wondering: Are those techniques and mental models still valid? Has Compose gotten smarter about handling performance on its own? Are there new tools I should be aware of?
...
As long as you pass the same object instance from your ViewModel, Strong Skipping will successfully skip the composable.
...
2.  **Use `@Immutable` for domain models.** It’s a cheap insurance policy for derived data.
3.  **Defer reads (Lambdas).** This remains the most effective way to isolate recomposition scopes.
4.  **Profile, don't guess.** Reach for compose tracing, the Compiler Metrics or Stability Analyzer if you actually see dropped frames.
---